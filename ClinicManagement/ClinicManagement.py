...
def add_doctor_patient_entry(KEY):
    ...
    try:
        ...
        if(KEY=='DOCTOR'):
            while True:
             name=Doctor(input("Enter name of doctor\n")),
             if not re.match("[A-Za-z]{2,25}", name):
                    break
            else:
                    print("Name entered is not valid")
            while True:        
             doctor=Doctor(input("Enter Id of doctor\n")),
             if not(re.match("[0-9]{1,9}", doctor)):
                    break
            else:
                    print("Id entered is not valid")

            while True:
             specialization=Doctor(input("Enter name of doctor\n")),
             if not re.match("[A-Za-z]{2,25}", specialization):
                    break
            else:
                    print("specialization entered is not valid")
                    
            while True:
             availability=Doctor(input("Enter Doctor Availability\n"))
             if not re.match("[A-Z]{2,4}", availability):
                    break
            else:
                    print("availability entered is not valid(capital letters only)")
          #  recordDictionar = {
          #  "doctorId": doctor,
           # "name": name,
        ...
        #}        
            ...
            doctorRecord={name:name.__dict__}
            doctorRecord={availability:availability.__dict__}
            doctorRecord={specialization:specialization.__dict__}
